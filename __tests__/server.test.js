...
describe('api server products', () => {
  it('should respond with 200 on a correct route', async () => {
    const results = await mockRequest.get('/api/v1/products');
    expect(results.status).toBe(200);
  });
  it('should respond with 302 on a wrong method', async () => {
    const results = await mockRequest.get('/api/v1/products/:id');
  ...
  it('should respond with 201 for post', async () => {
    ...
    const response = { name: 'Joe', id: 1, category: 'm', description: 'mm' };
